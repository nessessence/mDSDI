...
class Trainer_DSDI_Meta:
    ...
    def generate(self):
        ...
        for iteration in range(self.args.iterations):
            ...
            # idx = torch.randperm(samples.nelement())
            # samples = samples.view(-1)[idx].view(samples.size())
            ...
            if iteration % self.args.step_eval == 0:
                ...
                plt.savefig(self.plot_writer + "IMAGE_DEBUG_{}.png".format(iteration), bbox_inches='tight', dpi=600)


                z_gen = torch.zeros(samples.shape[0], self.args.feature_dim).cuda()
                di_z, ds_z, x_hat = self.model(samples, z_gen)
                for i in range(49):
                    # define subplot
                    plt.subplot(7, 7, 1 + i)
                    # turn off axis
                ...
                plt.savefig(self.plot_writer + "GEN_DEBUG_Zero_IMAGE_{}.png".format(iteration), bbox_inches='tight', dpi=600)
                ...
                plt.savefig(self.plot_writer + "GEN_DEBUG_One_IMAGE_{}.png".format(iteration), bbox_inches='tight', dpi=600)
                ...
                plt.savefig(self.plot_writer + "GEN_DEBUG_Two_IMAGE_{}.png".format(iteration), bbox_inches='tight', dpi=600)
                exit()
